...
type Scenario = Readonly<{
  ...
  approvedDocs: number;
  query?: { reason: string; remarks: string; requested: string[]; answered: boolean };
...
const SCENARIOS: readonly Scenario[] = [
  ...
  },
  {
    ...
    decision: {
      outcome: "REJECTED",
      remarks: "Account was not in the guarantee cover period at the date of default.",
    },
  },
];
...
function buildClaims(
  ...
  SCENARIOS.forEach((sc, n) => {
    ...
    claims.push({
      ...
      lastUpdatedAt: statusHistory[statusHistory.length - 1]?.at ?? ago(sc.daysAgo),
      bucket: sc.status === "DRAFT" || sc.status === "QUERY_RAISED" ? "LENDER" : "IMGC",
