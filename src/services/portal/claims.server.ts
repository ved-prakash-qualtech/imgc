...
export async function uploadDocument(
  ...
  if (session.role === "IMGC") {
    if (!session.isAdmin) return { ok: false, error: "Only a lender can upload documents." };
    const ctx = await getAdminContextOrNull();
    const docAccount = db.accounts.find(a => a.id === accountId);
    if (!ctx?.lenderOrgId || ctx.lenderOrgId !== docAccount?.lenderOrgId) {
      return { ok: false, error: "Admin context required to upload on behalf of a lender." };
    }
...
export async function deleteDocumentFile(
  ...
  if (session.role === "IMGC") {
    ...
    const docAccount = db.accounts.find(a => a.id === accountId);
    if (!ctx?.lenderOrgId || ctx.lenderOrgId !== docAccount?.lenderOrgId) {
      return { ok: false, error: "Admin context required to act on behalf of a lender." };
    }
